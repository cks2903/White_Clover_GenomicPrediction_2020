...
ggplot(Prediction_accuracies, aes(x=Trait, y=Accuracy,fill=Trait)) + 
  ...
  geom_point(aes(fill = Trait), size = 1, shape = 21, position = position_jitterdodge()) +
...
ggsave("PredictionAccuraciesOfDifferent_gpdTraits_20200924.pdf", width =20, height = 10, units = "cm")
#dev.off()
...
ggplot(Prediction_accuracies_true, aes(x=Trait, y=Accuracy,fill=Trait)) + 
  ...
  theme_classic() +theme(axis.text.x = element_text(size = 10, angle = 90,hjust=1),axis.text.y = element_text(size = 10), 
                         axis.title=element_text(size=15))
ggsave("PredictionAccuraciesOfDifferent_gpdTraits_20200924_realvalues.pdf", width =20, height = 10, units = "cm")
#dev.off()
...
ggplot(Prediction_accuracies_true, aes(x=Trait, y=Correlation,fill=Trait)) + 
  ...
  theme_classic() +theme(axis.text.x = element_text(size = 10, angle = 90,hjust=1),axis.text.y = element_text(size = 10), 
                         axis.title=element_text(size=15))
ggsave("PredictionCorrelationsOfDifferent_gpdTraits_20200924.pdf", width =20, height = 10, units = "cm")
