...
# Define functions for GBLUP
{
  GP_GBLUP<-function(testpop){
    ...
    matrix=cbind(as.character(d7_training$Clover),as.numeric(d7_training$gpd),as.numeric(GBLUP$ETA[[1]]$u))
    ...
    matrix1=cbind(as.character(d7$Clover[testpop]),as.numeric(d7$gpd[testpop]),as.numeric(as.character(GEBVpred)))
  ...
  GP_GBLUP_onBLUPs<-function(testpop){
    ...
    # Run the GBLUP model on full training population to extract GEBVs
    yNA=PseudoPhenotypes_training$BLUPs_gpd_AllGenetic
    ETA=list(list(K=GRM1_trn,model="RKHS"))
    GBLUP=BGLR(y=yNA,response_type = "gaussian",ETA=ETA,nIter=20000,burnIn = 5000,verbose=F,saveAt=paste("GBLUP",round))
    matrix=cbind(as.character(PseudoPhenotypes_training$Clover),as.numeric(PseudoPhenotypes_training$BLUPs_gpd_AllGenetic),as.numeric(GBLUP$ETA[[1]]$u))
    colnames(matrix)=c("ID", "Observed", "GEBV")
    GEBV=as.numeric(as.character(matrix[,3]))
    ...
    # Output matrix with prediction results
    matrix1=cbind(as.character(PseudoPhenotypes$Clover[testpop]),as.numeric(PseudoPhenotypes$BLUPs_gpd_AllGenetic[testpop]),as.numeric(as.character(GEBVpred)))
    colnames(matrix1)=c("ID", "Observed", "GEBV")
    return(matrix1)
  }
}
...
# Summarize and make files with results
{
  ...
  filename=paste("Correlation_GBLUPwithfixedeffects_gpd",round,".txt",sep="")
  write.table(correlation,filename,sep="\t")
  
  filename1=paste("Predictions_GBLUPwithfixedeffects_gpd",round,".txt",sep="")
...
# Summarize and make files with results
{
  ...
  filename=paste("Correlation_GBLUPonBLUPs_gpd",round,".txt",sep="")
  write.table(correlation2,filename,sep="\t")
  
  filename1=paste("Predictions_GBLUPonBLUPss_gpd",round,".txt",sep="")
