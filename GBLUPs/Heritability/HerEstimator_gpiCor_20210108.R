...
# Introduce new second_growth_phase_trait

newtrait = read.table("/Volumes/NAT_MBG-PMg/Cathrine/Nchain/Genomic_prediction_yield_July2020/V2_LessHarsh_SaraQualityFilter/New_fixation_Trait_20210106/GPD_day11to25.csv",head=T)
head(newtrait)

newtrait = newtrait[,c(1,13)]

merged_with_new_trait = merge(d6,newtrait,by.x="Barcode")

fit <- lm(GPD_in_interval ~ InitialSize, data=merged_with_new_trait) 
ycorr <- merged_with_new_trait$GPD_in_interval - model.matrix( ~ InitialSize, data=merged_with_new_trait) %*% fit$coefficients
merged_with_new_trait$gpi_cor <- ycorr #this is the new corrected dry weight
...
#  Calculate clover heritability
{
  ...
  Cloverfit <- bayz(gpi_cor ~ ranf(Clover) + fixf(NS) + fixf(EW) + fixf(Rhizobium) + fixf(inoculation_date) ,
               data = merged_with_new_trait, chain=c(20000, 5000, 10))
  
  summary(Cloverfit) # 26% broad sense clover

  
  avg_reps = nrow(merged_with_new_trait)/length(unique(merged_with_new_trait$Clovershort))
...
dim(GRM_rhi)
idx_to_keep=which(colnames(GRM_rhi) %in% unique(merged_with_new_trait$rhizobium))
rhiz_with_no_pheno=colnames(GRM_rhi)[-idx_to_keep]
idx_to_keep_2=which(unique(merged_with_new_trait$rhizobium) %in% colnames(GRM_rhi))
rhiz_with_no_geno=unique(merged_with_new_trait$rhizobium)[-idx_to_keep_2]

Rhiz_to_remove=(c(list(rhiz_with_no_pheno), list(as.character(rhiz_with_no_geno))))
Rhiz_to_remove=unlist(Rhiz_to_remove)
d6_new=merged_with_new_trait[-which(merged_with_new_trait$Rhizobium %in% Rhiz_to_remove),]
...
#  Calculate rhizobium heritability
{
  ...
  Rhizfit <- bayz(gpi_cor ~ ranf(Rhizobium) + fixf(NS) + fixf(EW) + fixf(Clover) + fixf(inoculation_date) ,
                    data = d6_new, chain=c(20000, 5000, 10))
...
#  Calculate clover heritability
{
  ...
  {
    gpdmeans=aggregate(merged_with_new_trait$gpi_cor, list(merged_with_new_trait$Clover), mean)
    colnames(gpdmeans)=c("Individual","gpi")
    
    iSizemeans=aggregate(merged_with_new_trait$InitialSize, list(merged_with_new_trait$Clovershort), mean)
  ...
  Cloverfit2 <- bayz(gpiCor ~ ranf(Individual,V=GRM1),
                    data = means_, chain=c(20000, 5000, 10))
...
#  Calculate rhiz heritability
{
  ...
  # calculate means of genotypes
  {
    gpdmeans=aggregate(d6_new$GPD_in_interval, list(d6_new$Rhizobium), mean)
    colnames(gpdmeans)=c("Individual","gpi")
    gpdmeans$Individual=droplevels(gpdmeans$Individual) # removing levels not used in actual data
  
    iSizemeans=aggregate(d6_new$InitialSize, list(d6_new$Rhizobium), mean)
    colnames(iSizemeans)=c("Individual","iSize")
  ...
  Rhizfit2 <- bayz(gpiCor ~ranf(Individual,V=GRM_rhi_filt),
                     data = means_, chain=c(20000, 5000, 10))
  ...
  FullModel <- bayz(gpi_cor ~ ranf(Clover) + fixf(NS) + fixf(EW) + ranf(Rhizobium) + fixf(inoculation_date) +ran2f(Rhizobium,Clover),
                    data = merged_with_new_trait, chain=c(20000, 5000, 10))
