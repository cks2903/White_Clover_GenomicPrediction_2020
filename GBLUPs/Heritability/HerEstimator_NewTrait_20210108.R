...
#  Calculate clover heritability
{
  ...
  Cloverfit <- bayz(GPD_in_interval ~ ranf(Clover) + fixf(NS) + fixf(EW) + fixf(Rhizobium) + fixf(inoculation_date) ,
...
#  Calculate rhizobium heritability
{
  ...
  Rhizfit <- bayz(GPD_in_interval ~ ranf(Rhizobium) + fixf(NS) + fixf(EW) + fixf(Clover) + fixf(inoculation_date) ,
...
#  Calculate clover heritability
{
  ...
  {
    gpdmeans=aggregate(merged_with_new_trait$GPD_in_interval, list(merged_with_new_trait$Clover), mean)
    colnames(gpdmeans)=c("Individual","gpdResCor")
  }
  
  Cloverfit2 <- bayz(gpdResCor ~ ranf(Individual,V=GRM1),
...
#  Calculate rhiz heritability
{
  ...
  Rhizfit2 <- bayz(gpdResCor ~ ranf(Individual,V=GRM_rhi_filt),
                     data = gpdmeans, chain=c(20000, 5000, 10))
...
FullModel <- bayz(GPD_in_interval ~ ranf(Clover) + fixf(NS) + fixf(EW) + ranf(Rhizobium) + fixf(inoculation_date) +ran2f(Rhizobium,Clover),
                  data = merged_with_new_trait, chain=c(20000, 5000, 10))
