...
#  Calculate clover heritability
{
  ...
  Cloverfit <- bayz(InitialSize ~ ranf(Clover) + fixf(NS) + fixf(EW) + fixf(Rhizobium) + fixf(inoculation_date) ,
...
#  Calculate rhizobium heritability
{
  ...
  Rhizfit <- bayz(InitialSize ~ ranf(Rhizobium) + fixf(NS) + fixf(EW) + fixf(Clover) + fixf(inoculation_date) ,
                    data = d6, chain=c(20000, 5000, 10))
...
#  Calculate clover heritability
{
  ...
  {
    gpdmeans=aggregate(d6$InitialSize, list(d6$Clovershort), mean)
    colnames(gpdmeans)=c("Individual","iSize")
  }
  
  Cloverfit2 <- bayz(iSize ~ ranf(Individual,V=GRM1),
  ...
  m$CloverAdVar=Cloverfit2$Estimates$postMean[3]
  m$CloverAdVar_SD=Cloverfit2$Estimates$postSD[3]
...
#  Calculate rhiz heritability
{
  ...
  # calculate means of genotypes
  {
    gpdmeans=aggregate(d6_new$InitialSize, list(d6_new$Rhizobium), mean)
    colnames(gpdmeans)=c("Individual","iSize")
    gpdmeans$Individual=droplevels(gpdmeans$Individual) # removing levels not used in actual data
    
  }
  
  Rhizfit2 <- bayz(iSize ~ ranf(Individual,V=GRM_rhi_filt),
                     data = gpdmeans, chain=c(20000, 5000, 10))
  ...
  m$h2 = (Rhizfit2$Estimates$postMean[2])/(Rhizfit2$Estimates$postMean[2]+Rhizfit2$Estimates$postMean[1])
...
FullModel <- bayz(InitialSize ~ ranf(Clover) + fixf(NS) + fixf(EW) + ranf(Rhizobium) + fixf(inoculation_date) +ran2f(Rhizobium,Clover),
                  data = d6, chain=c(20000, 5000, 10))
