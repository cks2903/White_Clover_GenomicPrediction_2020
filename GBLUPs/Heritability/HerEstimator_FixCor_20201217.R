...
#  Calculate clover heritability
{
  ...
  Cloverfit <- bayz(gpd_dryweight_cor ~ ranf(Clover) + fixf(NS) + fixf(EW) + fixf(Rhizobium) + fixf(inoculation_date) ,
...
#  Calculate rhizobium heritability
{
  ...
  Rhizfit <- bayz(gpd_dryweight_cor ~ ranf(Rhizobium) + fixf(NS) + fixf(EW) + fixf(Clover) + fixf(inoculation_date) ,
                    data = d6, chain=c(20000, 5000, 10))
...
#  Calculate clover heritability
{
  ...
  # Correct GPD for the full effect of initial size after averaging
  {
    ...
    fit <- lm(gpd ~ iSize, data=means_)   
    ycorr <- means_$gpd - model.matrix( ~ iSize, data=means_) %*% fit$coefficients
    means_$gpdFixCor <- ycorr #this is the new corrected dry weight
  }
  
  
  Cloverfit2 <- bayz(gpdFixCor ~  ranf(Individual,V=GRM1),
...
#  Calculate rhiz heritability
{
  ...
  Rhizfit2 <- bayz(gpdFixCor ~ ranf(Individual,V=GRM_rhi_filt),
                   data = means_, chain=c(20000, 5000, 10))
...
FullModel <- bayz(gpd_dryweight_cor ~ ranf(Clover) + fixf(NS) + fixf(EW) + ranf(Rhizobium) + fixf(inoculation_date) +ran2f(Rhizobium,Clover),
                  data = d6, chain=c(20000, 5000, 10))
