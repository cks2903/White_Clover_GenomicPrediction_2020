...
removereplicates <- function(maxreplicates,dataframe){
  ...
  for (genotype in Genotypes){
    ...
    colnames(gpdmeans)=c("Clovershort","gpd")
  }
...
GP_GBLUP<-function(testpop){
  ...
  gpdmeans_training_ready=na.omit(gpdmeans_training, cols = c("gpd")) # remember that gpd na inidividuals should be removed whether or not they are in the training pop or not
  ...
  yNA=gpdmeans_training_ready$gpd
  ...
  matrix=cbind(as.character(gpdmeans_training_ready$Clovershort),as.numeric(gpdmeans_training_ready$gpd),as.numeric(GBLUP$ETA[[1]]$u))
  ...
  matrix1=cbind(as.character(dataframe$Clovershort[testpop]),as.numeric(dataframe$gpd[testpop]),as.numeric(as.character(GEBVpred)))
