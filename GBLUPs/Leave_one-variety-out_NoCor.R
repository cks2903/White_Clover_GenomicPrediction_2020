...
{
  Correctedforallfixed <- lmer(growth_per_day ~ factor(Round) + factor(NS) + factor(EW) + factor(Rhizobium) + factor(inoculation_date) + (1|Clover), data=d6) 
...
GP_GBLUP<-function(testpop){
  ...
  additional_individuals_to_remove=which(is.na(d6$growth_per_day)==TRUE) #no genotypes
...
d6_without_NA=na.omit(d6, cols = c("growth_per_day")) # remove individuals with no gpd information from data
d6_without_NA$Clover=droplevels(d6_without_NA$Clover) # removing levels not used in actual data

y=d6_without_NA$growth_per_day
