...
GP_GBLUP<-function(testpop){
  ...
  gpdmeans_training=merged[-testpop,] # limit the dataframe to only the individuals allowed for training the model
  ind_not_in_train=merged$Individual[testpop]
  ...
  yNA=gpdmeans_training$gpdFixCor
  ...
  matrix=cbind(as.character(gpdmeans_training$Individual),as.numeric(gpdmeans_training$gpdFixCor),as.numeric(GBLUP$ETA[[1]]$u))
  ...
  matrix1=cbind(as.character(merged$Individual[testpop]),as.numeric(merged$gpdFixCor[testpop]),as.numeric(as.character(GEBVpred)))
...
# calculate means of genotypes
{
  ...
  merged=merge(gpdmeans,iSizemeans,by="Individual")
}

# Correct GPD for the full effect of initial size
{
  fit <- lm(gpd ~ iSize, data=merged) 
  ycorr <- merged$gpd - model.matrix( ~ iSize, data=merged) %*% fit$coefficients
  merged$gpdFixCor <- ycorr #this is the new corrected dry weight
}
...
if (is.na(args[3])){
  set.seed(NULL)
  tst=sample(1:length(unique(merged$Individual)),size=length(unique(merged$Individual)),replace=FALSE) 
  k=6
  ...
  testpop1=unique(merged$Individual)[tst1]
  testpop2=unique(merged$Individual)[tst2]
  testpop3=unique(merged$Individual)[tst3]
  testpop4=unique(merged$Individual)[tst4]
  testpop5=unique(merged$Individual)[tst5]
  testpop6=unique(merged$Individual)[tst6]
  
  testpop1_idx=which(merged$Individual %in% testpop1)
  testpop2_idx=which(merged$Individual %in% testpop2)
  testpop3_idx=which(merged$Individual %in% testpop3)
  testpop4_idx=which(merged$Individual %in% testpop4)
  testpop5_idx=which(merged$Individual %in% testpop5)
  testpop6_idx=which(merged$Individual %in% testpop6)
