...
GP_GBLUP<-function(testpop){
  ...
  d6_training_ready=na.omit(d6_training, cols = c("InitialSize")) # remember that gpd na inidividuals should be removed whether or not they are in the training pop or not
  ...
  yNA=d6_training_ready$InitialSize
  
  fixedmod=model.matrix(~factor(d6_training_ready$NS)+factor(d6_training_ready$EW)+factor(d6_training_ready$Rhizobium)+factor(d6_training_ready$inoculation_date))
  ...
  print(d6$CorrectedPheno[testpop])
  print(d6$InitialSize[testpop])
  return(matrix1)
}
}
...
# Load data
{
  ...
  #d <- read.csv("/Volumes/NAT_MBG-PMg/Cathrine/Nchain/Genomic_prediction_yield_July2020/V2_LessHarsh_SaraQualityFilter/GBLUP_iSize_20200925/greenhouse_area.csv", header = TRUE, sep = ",")
  ...
  #f=read.csv("/Volumes/NAT_MBG-PMg/Cathrine/Nchain/Genomic_prediction_yield_July2020/V2_LessHarsh_SaraQualityFilter/GBLUP_iSize_20200925/2018_weight.csv",header=T,sep=";")
...
# Load genomic relationship matrix and make sure clover genotypes match data
{
  ...
  #GRM=read.table("/Volumes/NAT_MBG-PMg/Cathrine/Nchain/Genomic_prediction_yield_July2020/V2_LessHarsh_SaraQualityFilter/GBLUP_iSize_20200925/GRM_Clover_Fullfiltering_20200728.csv",sep=",",header=T)
  dim(GRM)
...
{
  Correctedforallfixed <- lmer(InitialSize ~ factor(NS) + factor(EW) +factor(Rhizobium) + inoculation_date + (1|Clover), data=d6) 
  summary(Correctedforallfixed)
  matrixOfeffects=model.matrix( ~ factor(NS) + factor(EW) +factor(Rhizobium) + inoculation_date, data=d6)
  ...
  ycorr <- d6$InitialSize - matrixOfeffects_new %*% fixef(Correctedforallfixed)
...
# Summarize and make files with results
{
  ...
  All1=aggregate(as.numeric(as.character(All[, 2])), list(All[,1]), mean)
  All2=aggregate(as.numeric(as.character(All[, 3])), list(All[,1]), mean)
...
# write output phenotypes for GWAS
{
  #iSize_NotCor=aggregate(as.numeric(as.character(d6$InitialSize)), list(d6$Clovershort), mean)
  #iSize_Cor=aggregate(as.numeric(as.character(d6$CorrectedPheno)), list(d6$Clovershort), mean)

  #phenofile=merge(iSize_NotCor,iSize_Cor,by="Group.1")
  #colnames(phenofile)=c("Accessions","Size_NotCor","iSize_Cor")
  #write.table(phenofile,"iSize_phenotype.csv",sep=",",row.names = F, col.names = T,quote=F)
}
