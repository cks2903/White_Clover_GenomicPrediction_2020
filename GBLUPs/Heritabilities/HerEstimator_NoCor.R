...
# Calculate a phenotype corrected for all fixed effects
{
  Correctedforallfixed_rhiz <- lmer(growth_per_day ~ factor(Round) + factor(NS) + factor(EW) + (1|Rhizobium) + inoculation_date + factor(Clover), data=d6_new) 
  ...
  ycorr <- d6_new$growth_per_day - matrixOfeffects_new %*% fixef(Correctedforallfixed_rhiz)
