####################################################################################
# Prediction of F1 clover generation.      iSize                                   #
####################################################################################
#                       GBLUP                                                      #
####################################################################################
...
# Fix data matrix so that names are the same as in GRM, that is 118 individuals in each
{
  ...
  length(unique(d4$Clovershort)) #147 unique genotypes with iSize data
...
fakedata1=c(rep("NA",7),"Aoost_03",rep("NA",8),"Aoost_03",rep("NA",8),"Aoost_03")
fakedata2=c(rep("NA",7),"Aoost_06",rep("NA",8),"Aoost_06",rep("NA",8),"Aoost_06")
fakedata3=c(rep("NA",7),"Aaran_06",rep("NA",8),"Aaran_06",rep("NA",8),"Aaran_06")
fakedata4=c(rep("NA",7),"Kdike_10",rep("NA",8),"Kdike_10",rep("NA",8),"Kdike_10")
d6=rbind(d6,fakedata1,fakedata2,fakedata3,fakedata4)
d6=d6[order(d6$Clovershort),] 
d6$InitialSize=as.numeric(as.character(d6$InitialSize))
...
{
  d6$InitialSize = as.numeric(as.character(d6$InitialSize))
  iSizemeans=aggregate(d6$InitialSize, list(d6$Clovershort), mean)
...
# Divide into F1 parental groups
{
  ...
  parentpop1names_idx=which(iSizemeans$Individual%in%parentpop1names)
  parentpop2names_idx=which(iSizemeans$Individual%in%parentpop2names)
  parentpop3names_idx=which(iSizemeans$Individual%in%parentpop3names)
  parentpop4names_idx=which(iSizemeans$Individual%in%parentpop4names)
  parentpop5names_idx=which(iSizemeans$Individual%in%parentpop5names)
  parentpop6names_idx=which(iSizemeans$Individual%in%parentpop6names)
  parentpop7names_idx=which(iSizemeans$Individual%in%parentpop7names)
  parentpop8names_idx=which(iSizemeans$Individual%in%parentpop8names)
  parentpop9names_idx=which(iSizemeans$Individual%in%parentpop9names)
...
GP_GBLUP<-function(testpop){
  ...
  iSizemeans_training=iSizemeans[-testpop,] # limit the dataframe to only the individuals allowed for training the model
  print(paste("this is the dimensions of training phenotypes",dim(iSizemeans_training),sep=" "))
  iSizemeans_training_ready_pre=na.omit(iSizemeans_training, cols = c("iSize")) # do not use individuals with no phenotypes for training
  additional_individuals_to_remove=which(iSizemeans_training_ready_pre$Individual %in% c("Aoost_03","Aoost_06","Aaran_06","Kdike_10")) # do not use individuals with no genotypes for training
  if (length(additional_individuals_to_remove)!=0){
    iSizemeans_training_ready = iSizemeans_training_ready_pre[-additional_individuals_to_remove,]
  }else{
    iSizemeans_training_ready = iSizemeans_training_ready_pre
  }
  print(paste("Using",nrow(iSizemeans_training_ready), "Individuals to train the model",sep=" "))
  
  ind_not_in_train=iSizemeans$Individual[testpop]
  IndividualsToRemoveGRM=which(colnames(GRM1) %in% ind_not_in_train)
  
  
  GRM_trn = GRM1[colnames(GRM1) %in% iSizemeans_training_ready$Individual,colnames(GRM1) %in% iSizemeans_training_ready$Individual]

  # Run the GBLUP model on full training population to extract GEBVs
  yNA=iSizemeans_training_ready$iSize
  ...
  matrix=cbind(as.character(iSizemeans_training_ready$Individual),as.numeric(iSizemeans_training_ready$iSize),as.numeric(GBLUP$ETA[[1]]$u))
  ...
  GRMforpred_covar = GRM1[testpop,colnames(GRM1) %in% iSizemeans_training_ready$Individual] # Covariance between training and testing pop.
  
  GEBVpred = GRMforpred_covar%*%ginv(GRM_trn) %*% GEBV_contribution1data 

  # Output matrix with prediction results
  matrix1=cbind(as.character(iSizemeans$Individual[testpop]),as.numeric(iSizemeans$iSize[testpop]),as.numeric(as.character(GEBVpred)))
...
filename1="Predictions_GBLUP_iSize.txt"
...
filename2="Correlations_GBLUP_iSize.txt"
