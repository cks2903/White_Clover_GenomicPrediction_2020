...
Correcterfunction<-function(dataframe){
  ...
  # of replicates
  {
    lm.fit <- lm(dataframe$InitialSize ~ dataframe$Clover)
    summary(lm.fit)
    dataframe$residuals <- round(lm.fit$residuals,2)
    
    fit <- lm(growth_per_day ~ residuals, data=dataframe) 
    ycorr <- dataframe$growth_per_day - model.matrix( ~ residuals, data=dataframe) %*% summary(fit)$coefficients[,1]
